...
// Adapted by lyn to be a sprite for CS111 Spring '03

import java.awt.*;       // Import Abstract Window Toolkit

abstract class TurtleSpriteNew extends Sprite {
 ...
 // Something required for sprites but not normal turtles
 ...
 */
 ...
 private int screenX(double xcoord) {
  return (int) xcoord; 
//(int) Math.round(bounds.x + (bounds.width / 2) + xcoord);
 }
 
 private int screenY(double ycoord) {
  return (int) ycoord; 
//(int) Math.round(bounds.y + (bounds.height / 2) - ycoord);
 }
